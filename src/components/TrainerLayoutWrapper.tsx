"use client";

import { useState, useLayoutEffect } from "react";
import { useRouter, usePathname } from "next/navigation";
import Link from "next/link";
import TrainerSidebar from "./TrainerSidebar";
import Logo from "./Logo";
import AIAssistant from "./AIAssistant";

interface Props {
  children: React.ReactNode;
}

const CLIENT_NAV = [
  { href: "/home", label: "Home", icon: "ti-home" },
  { href: "/workout", label: "Workout", icon: "ti-barbell" },
  { href: "/nutrition", label: "Nutrition", icon: "ti-salad" },
  { href: "/log", label: "Log", icon: "ti-plus-circle" },
  { href: "/progress", label: "Progress", icon: "ti-chart-line" },
  { href: "/settings", label: "Settings", icon: "ti-settings" },
];

export default function TrainerLayoutWrapper({ children }: Props) {
  const [clientMode, setClientMode] = useState(false);

  useLayoutEffect(() => {
    try {
      const saved = localStorage.getItem("symmetry_client_mode");
      const cookieVal = document.cookie
        .split(";")
        .find(c => c.trim().startsWith("symmetry_client_mode="))
        ?.split("=")[1]
        ?.trim();
      const effective = saved ?? cookieVal;
      setClientMode(effective === "1");
    } catch {}
  }, []);

  const pathname = usePathname();

  function handleToggleMode() {
    const next = !clientMode;
    setClientMode(next);
    try {
      localStorage.setItem("symmetry_client_mode", next ? "1" : "0");
    } catch {}
    if (next) {
      document.cookie = "symmetry_client_mode=1; path=/; max-age=86400; SameSite=Lax";
    } else {
      document.cookie = "symmetry_client_mode=; path=/; max-age=0; SameSite=Lax";
    }
    // Full reload so server reads the new cookie and renders correct view
    window.location.href = "/home";
  }

  function isActive(href: string) {
    if (href === "/home") return pathname === "/home" || pathname === "/";
    return pathname.startsWith(href);
  }

  if (clientMode) {
    return (
      <div className="flex flex-col min-h-screen" style={{ background: "var(--brand-bg)" }}>
        {/* Header */}
        <div
          className="flex items-center gap-3 px-4 pb-3 sticky top-0 z-40 shadow-sm"
          style={{
            background: "var(--brand-primary)",
            paddingTop: "calc(12px + env(safe-area-inset-top))",
          }}
        >
          <Logo size={28} color="white" className="flex-shrink-0" />
          <div className="flex-1">
            <span className="text-white font-semibold text-sm">Symmetry</span>
            <span className="text-white/50 text-xs ml-2">· My Training</span>
          </div>
          <button
            onClick={handleToggleMode}
            className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg"
            style={{ background: "rgba(255,255,255,0.15)", color: "white" }}
          >
            <i className="ti ti-layout-dashboard text-sm" />
            Trainer View
          </button>
        </div>

        {/* Page content */}
        <div className="flex-1 pb-20 overflow-y-auto">{children}</div>

        {/* Bottom nav — matches BottomNav.tsx exactly */}
        <nav
          className="fixed bottom-0 left-0 right-0 z-40 flex"
          style={{
            background: "var(--brand-surface)",
            borderTop: "1px solid var(--brand-border)",
            paddingBottom: "env(safe-area-inset-bottom)",
          }}
        >
          {CLIENT_NAV.map(item => {
            const active = isActive(item.href);
            return (
              <Link
                key={item.href}
                href={item.href}
                className="flex-1 flex flex-col items-center gap-1 py-3 transition-colors"
                style={{ color: active ? "var(--brand-primary)" : "var(--brand-text-secondary)" }}
              >
                <i className={`ti ${item.icon} text-xl`} />
                <span className="text-[10px] font-medium">{item.label}</span>
              </Link>
            );
          })}
        </nav>
      </div>
    );
  }

  return (
    <div className="flex min-h-screen" style={{ background: "var(--brand-bg)" }}>
      <TrainerSidebar
        clientMode={clientMode}
        onToggleClientMode={handleToggleMode}
        userName="Dustin Gautreaux"
        userInitials="DG"
      />
      <div className="flex-1 min-w-0 overflow-y-auto">
        <div className="lg:hidden h-14" />
        {children}
      </div>
      <AIAssistant />
    </div>
  );
}
