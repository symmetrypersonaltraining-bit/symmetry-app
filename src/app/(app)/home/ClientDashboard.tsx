...
import DailyMacrosRing from "@/components/DailyMacrosRing";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: "rgba(0,0,0,0.5)" }} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="w-full max-w-lg rounded-t-3xl p-6 pb-8" style={{ background: "var(--brand-bg)", maxHeight: "85vh", overflowY: "auto" }}>
        ...
        <Link href="/progress" onClick={onClose} className="flex items-center justify-center gap-2 mt-5 py-3 rounded-3xl text-sm font-semibold" style={{ background: "var(--brand-primary)", color: "white" }}><i className="ti ti-chart-line" />View Full Progress</Link>
      </div>
    </div>
  );
}
...
{todayWorkouts.length > 0 ? (
        ...
        <Link
          href="/settings"
          className="w-full py-3 rounded-3xl text-sm font-medium mt-2 flex items-center justify-center"
