...
function WeekRing({
  ...
  const displayWeekStartStr = displayWeekStart.toISOString().split("T")[0];
        ...
        {weekDays.map(({ dow, dateStr, workout, isToday }) => {
          ...
          return (
            <div key={dow} className="flex flex-col items-center gap-1">
              {workout?.id ? (
                <Link href={`/workout/${workout.id}${clientId ? `?forClient=${clientId}` : ``}`}>{circle}</Link>
              ) : circle}
...
export default function ClientDashboard({
  ...
  recentWorkouts, streakDays, weekWorkouts, allScheduled = [], clientId,
}: Props) {
  ...
  return (
    <>
      ...
      <div className="p-4 pb-28 space-y-4 max-w-lg mx-auto">
        ...
        {todayWorkout ? (
          <Link href={`/workout/${todayWorkout.id}${clientId ? `?forClient=${clientId}` : ``}`}>
          ...
          {recentWorkouts.length === 0 ? (
            ...
            <div className="rounded-2xl overflow-hidden"
              ...
              {recentWorkouts.map((w, i) => (
                <Link key={w.id} href={`/workout/${w.id}${clientId ? `?forClient=${clientId}` : ``}`}>
                  <div
