...
// applyProposal, the day-tree loader and their helpers live in lib/ai/workoutAdjust
// and are shared with the trainer agent. This route carried a byte-identical
// SECOND copy of all of it, so a fix to one silently left the other behind —
// which is exactly what happened with undo: the agent's path grew an audit
// trail and this one did not. One copy now.
import {
  applyProposal, loadDayTree, upcomingSessionsForDay, CT_TODAY,
...
export async function POST(req: NextRequest) {
  ...
  if (body.apply && body.apply.scheduled_workout_id && Array.isArray(body.apply.changes)) {
    const applyScope = body.applyScope === "series" ? "series" : "one";
    const res = await applyProposal(admin, clientId, body.apply, applyScope);
    // The same audit row the agent writes, so "undo that" reaches an edit made
    // from the AI drawer too. Without it, whether a change could be reversed
    // depended on which of two identical code paths happened to run it.
    if (res.ok) {
      try {
        await admin.from("ai_action_log").insert({
          actor: "workout_assist",
          action: "adjust_workout",
          client_id: clientId,
          summary: `${body.apply.summary || "Workout adjustment"} (${applyScope})`,
          undo: res.undo,
        });
      } catch (e) {
        // The workout has already changed. Losing the audit row is bad; telling
        // the trainer the change failed when it did not is worse.
        console.error("workout-assist: action log failed", e);
      }
    }
