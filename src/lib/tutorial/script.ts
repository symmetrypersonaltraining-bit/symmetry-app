...

  {
    ...
    steps: [
      ...
      },
      {
        ...
        narration:
          "Each client carries one focus line for the week, and their app shows it. It is written late on Saturday night, after the week has finished, from that client's real numbers. Underneath it, their app now shows the coach's read — two or three sentences on how the training actually went, from the same pass. It publishes straight to them, and there is nothing for you to approve. Write one yourself and the automatic one leaves it alone that week. It only writes about what it can see: a client who has never logged food is not told about food, and one who logged and stopped gets pushed to start again. The card on Home tells you whether it worked, with the lines one tap away. If it ever fails you get an email, and a client sees no focus rather than last week's, because a stale line presented as this week's is worse than none.",
        route: "/home",
        routeLabel: "Open Home",
        manual: [
