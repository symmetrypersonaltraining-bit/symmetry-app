...
<div class="mk-note mk-decide">
  <h2>One thing I need you to rule on</h2>
  <p><b>The next meal due: bright in place, or hoisted to the top?</b></p>
  <p>On the Workout tab, today is lifted out of date order and drawn first,
  because a week is a list you scan and the one you want is buried in it.</p>
  <p>A day of meals is not that. M1 → M6 <em>is</em> the order you eat in, and
  hoisting M3 above M1 would break the only thing that makes the list readable —
  and it would move as the day went on.</p>
  <p><b>So the mock-up leaves the next meal in place</b> and marks it another
  way: the bright fill goes to the day summary at the top (the real "now" of
  this screen), and M3 carries a <em>NEXT</em> label in its meta line.</p>
  <p>Say the word and I will hoist it instead — but that is the one place this
  screen deliberately does not copy the Workout tab, and I would rather you
  decided that than assumed it.</p>
</div>
