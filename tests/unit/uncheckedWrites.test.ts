...
const ALLOWED: Record<string, { max: number; why: string }> = {
  ...
  // src/lib/logClientError.ts used to sit here with one unchecked insert. It no
  // longer writes at all — it hands off to /api/log-error, whose writes ARE
  // checked (lib/errorLog.ts), because that route answers { ok } to the browser
  // and an unchecked write would have it say yes to a refusal.
